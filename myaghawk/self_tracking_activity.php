...
if (isset($_GET['export']) && $_GET['export'] == "csv") {
    exportSelfTrackingActivities($conn); // Call the function to export
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $_POST['action']=='add_self_tracking_activity') {
    add_self_service_request($account_id, $parcel_id, $block_id, $_POST, $service_type, $frequency, $recurrence_end_date);
}
...
<section class="userSection">
    ...
    <div class="pageHdrActions serviceRequests">
        ...
        <a target="_blank" class="exportBtn" href="export_self_service_requests.php" title="Export Records"><i class="fa-solid fa-file-export"></i> Export</a>
    </div>
...
$(document).ready(function () {
    ...
    function fetchSelfServiceRequests() {
        let parcelId = $("#parcel_id").val();
        let blockId = $("#block_id").val();
        let status = $("#status").val();

        $.ajax({
            url: "_fetch_self_service_requests.php",
            type: "GET",
            ...
            success: function (response) {
                ...
                if (response.length > 0) {
                    ...
                    tbody.append(
                        `<tr>
                            <td colspan="6" class="no-results">No self tracking activity found. <a href="self_tracking_add">Create a new self-tracking activity</a>.</td>
                        </tr>`
                    );
                }
            },
    ...
    function updateExportLink() {
        ...
        let exportUrl = "export_self_service_requests.php";
        if (params.toString()) {
    ...
    fetchSelfServiceRequests(); // Auto-fetch service requests on page load

    // Event listeners
    $("#parcel_id, #block_id, #status").change(fetchSelfServiceRequests);
